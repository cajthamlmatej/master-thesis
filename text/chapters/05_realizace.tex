...
% - v editoru lze nastavit nastaveni mimo jine:
%  - průchod: automaticky (po case na snimku), rucne a nebo pouze interaktivitou
%  - zobrazeni: vzdy roztahle na stranku, nemoznost manipulace a nebo moznost hybat s platnem
% - po zapnuti přehravače je vždy v základním rezimu prezentovani
% - prezentovnani lze potom zapnout do modu sledovani
% - prezentujici dostane odkaz, kod a QR kod, ktery muze sdilet ostatnim
% - ostatni se mohou pripojit, s materialem interaguji jen pomoci bloku, pohyb v snimku nejde
% - potom se synchronizuje aktualni snimek a kresleni
% - lze pouzit vzdalenou komunikaci, viz pozdejsi sekce \ref{text:realizace/vzdalenaKomunikace}, a tedy lze vytvorit zcela interaktivni prvky, jako chat ci ruzne kvizy
...
Ukázku typů lze najít v kódu \ref{code:realizace/typy}, celý soubor typů lze najít ve složce \verb|/plugins/types.d.ts|\todo{ověřit}.
...
% - pomoci docosaurus
% - dve casti: pouziti a rozsirovani
% - pouziti: ui aplikace, rozdeleni, ruzne funkcionality
% - rozsirovani: rozsirovani kodu a pomoci pluginu, lokalni instalace, pluginy a jejich api, dokumentace, postup tvorby pluginu postupne
% - psani v markdownu, hodne propojeni pro prehlednost
% - psano v anglictine
...
% - docker compose easy
% - docker compose v rootu
% - kazda cast ma svuj dockerfile
% - propojeni klicovych promennych pomoci globalniho .env
% - compose predava kazde casti tyto promenne a pridava sve na propojeni mezi castmi

% \subsection{Nasazení na produkci}

% - koupena domena
% - vlastni server kde vse hostuji
% - docker-compose
% - automaticke nahrani pomoci gitlab actions, pouzivam ssh s rsync, na serveru se zbuilduje
% - nasazeni bez vypadku
% - masked promenne, bezpecne

% \subsection{Vývoj}

% - specialni docker compose a specialni dockerfile pro kazdou casti 
% - nemountuje se, jen na nodemodules,  
% - staci spustit a vse jede
...
% - všechny pluginy jsou k dispozici v nahrané aplikaci a v příloze 
% \subsection{Centrist}
...
% Přo testování systému jsem si vytvořil pár úkazkových materiálů
% - ukázka systému, možnost vytvořit komplexní prezentace do jdou v jiných aplikacích
% - více v testování\todo{dopsat}