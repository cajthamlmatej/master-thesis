...
Diagram návrhu architektury je vyobrazen v obrázku~\ref{fig:architektura}.\todo{dokumentace, e-mail service}

\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.85\textwidth]{media/04_navrh/architektura.pdf}
...
Možné je používat např. metody polling, long polling, server-sent events a další~\todo{cite}, které běží přímo za pomoci protokolu HTTP.
Vhodnější je dle mého použít protokol WebSocket, který je přesně pro real-time oboustrannou komunikaci určen~\todo{cite}.
Na serveru lze používat různé knihovny.
Mezi nejznámější~\todo{cite} patří např. \verb|ws| či \verb|socket.io|.
...
Celkem vzniklo 22\todo{Zkontrolovat} návrhových obrazovek, jejichž ukázky jsou uvedeny v obrázcích \ref{fig:navrhovaObrazovkaRozsireni} a \ref{fig:navrhovaObrazovkaEmailAKod}, zatímco úplný přehled všech návrhů lze nalézt v příloze \ref{appendix:navrhoveObrazovky}.
Mezi ně patří:\todo{navrhove obrazovky hlavni stranky? nejak rozlisti}
