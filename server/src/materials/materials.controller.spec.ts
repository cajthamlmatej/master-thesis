...
import { StreamableFile, BadRequestException, UnauthorizedException } from '@nestjs/common';
...
describe('MaterialsController', () => {
  ...
  describe('findAllForUser', () => {
    ...
    it('should return materials for user', async () => {
      ...
      const materials = [
        { id: 'm1', name: 'Mat1', createdAt: now, updatedAt: now, slides: [{ thumbnail: 't1' }], user: 'user1', featured: false },
      ];
      ...
      expect(result).toEqual({ materials: [
        { id: 'm1', name: 'Mat1', createdAt: now, updatedAt: now, thumbnail: 't1', user: 'user1', featured: false }
      ] });
    });
  });
  ...
  describe('findOne', () => {
    ...
    beforeEach(() => {
      materialDoc = {
        ...
        updatedAt: now,
        user: 'user1',
        slides: [{ id: 's1', thumbnail: 't1', position: 0, data: { editor: { size: { width: 100, height: 100 }, color: 'white' }, blocks: [] } }],
        attendees: [],
        featured: true,
  ...
  describe('create', () => {
    it('should create and return material', async () => {
      const now = new Date();
      const createDto = { name: 'New', plugins: [{ plugin: 'p1', release: 'r1' }], visibility: 'PUBLIC', method: 'MANUAL', automaticTime: 500, sizing: 'FIT_TO_SCREEN', slides: [] } as any;
      const saved = { id: 'm2', name: 'New', plugins: [{ plugin: 'p1', release: 'r1' }], visibility: 'PUBLIC', method: 'MANUAL', automaticTime: 500, sizing: 'FIT_TO_SCREEN', user: 'u1', createdAt: now, updatedAt: now, slides: [], featured: false } as any;
