...
describe('EventsGateway', () => {
  ...
  describe('joinPlayerMaterialRoom', () => {
    it('should throw exception when material is not found', async () => {
      ...
      await expect(gateway.handleJoinPlayer({ materialId: 'non-existent-id', code: 'code', slideId: 'slide-id-1' }, client))
    ...
    it('should throw exception when user is not authorized', async () => {
      ...
      await expect(gateway.handleJoinPlayer({ materialId: 'material-id-1', code: 'code', slideId: 'slide-id-1' }, client))
        .rejects.toThrow(WsException);
      await expect(gateway.handleJoinPlayer({ materialId: 'material-id-1', code: 'code', slideId: 'slide-id-1' }, client))
  ...
  describe('blockMessage handling', () => {
    it('should throw exception when sending block message without being in player room', async () => {
      ...
      await expect(gateway.handleSendBlockMessage({ message: 'test', blockId: 'block-1' }, client)).rejects.toThrow(WsException);
      await expect(gateway.handleSendBlockMessage({ message: 'test', blockId: 'block-1' }, client)).rejects.toThrow('You are not in the player room');
    });
    ...
    it('should throw exception when presenter tries to send block message', async () => {
      ...
      await expect(gateway.handleSendBlockMessage({ message: 'test', blockId: 'block-1' }, client)).rejects.toThrow(WsException);
      await expect(gateway.handleSendBlockMessage({ message: 'test', blockId: 'block-1' }, client)).rejects.toThrow('You are the presenter');
    });
