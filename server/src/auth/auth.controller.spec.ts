...
describe('AuthController', () => {
  ...
  const mockUser = { _id: 'user-id', name: 'Test User', email: 'test@example.com', token: 'verify-token', active: false } as any;
  ...
  beforeEach(async () => {
    ...
    configService = ({
      get: jest.fn((key: string) => {
        switch (key) {
          case 'FRONTEND_DOMAIN': return 'http://app.com';
          case 'FRONTEND_DOMAIN_AUTHENTICATION': return '/login';
          case 'FRONTEND_DOMAIN_AUTHENTICATION_VALIDATE': return '/activate/{{token}}';
          default: return '';
        }
      }),
